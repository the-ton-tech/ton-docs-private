...
Wallet recognizes and processes the `e` parameter from the connect URL —
...
`.toUint8Array()` to materialize the decoded bytes once.
...
URL-safe normalization (i.e. `nacl.decodeBase64` rejects it).
