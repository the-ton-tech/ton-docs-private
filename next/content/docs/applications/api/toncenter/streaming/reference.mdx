...
| `state`    | `AccountState` | Account state without full code and data [BoCs](/blockchain-basics/primitives/serialization/boc). Includes state hash, nanoToncoin balance, [account status](/foundations/status), data and code hashes. All hashes are given in the Base64 format. |
