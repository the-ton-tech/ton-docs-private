...
Specify the structure of files and directories inside the [`items` property](#items) as a JavaScript list of strings, objects, and nested lists. No imports are needed — the component is available globally.
...
For cases where a data-driven approach is unnecessary, use the `<Tree>` component directly. It accepts `<Tree.File>` and `<Tree.Folder>` sub-components as children.
