...
Display directory structures with collapsible subdirectories using `<Tree.Folder>` and `<Tree.File>` sub-components. Supports MDX content inside children. See [detailed documentation](/overview/contribute/snippets/tree).
...
These components were evaluated and do not fit the documentation UI:

1. `<FileTree>` — data-driven variant, replaced by [`<Tree>`](#tree) which supports native MDX content.
1. `<SourceCodeLink>` — pill-button style does not fit inline text, tables, or lists. Use regular Markdown links instead.
1. `<RequestExample>` / `<ResponseExample>` — create a nested "field inside field" effect. Use fenced code blocks with `title` instead.
1. `<CommandOptions>` / `<CommandOption>` — existing tables and lists for CLI flags are clearer.
