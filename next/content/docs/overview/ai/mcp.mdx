...
By default, the server starts in the [agentic wallets mode](#agentic-wallets-mode). For the [single-wallet mode](#single-wallet-mode), add `MNEMONIC` or `PRIVATE_KEY` to the `env` object.
