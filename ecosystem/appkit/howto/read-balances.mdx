...
Each jetton holder has a dedicated jetton wallet contract derived from `(jettonMaster, ownerAddress)`. For display purposes — for example, showing the user their per-jetton wallet address — resolve it explicitly. `transferJetton` handles this derivation internally, so you do not need to pre-resolve the address to send.
