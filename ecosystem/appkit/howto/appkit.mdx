...
A single `AppKit` instance holds configured networks, registered connectors, connected wallets, and managers for swap, staking, and streaming flows. Anything that reads or changes wallet state should go through this instance.

## Configuration

Create the `AppKit` instance once with an `AppKitConfig`. All fields are optional; pass only the configuration your app needs. See [Connectors](/ecosystem/appkit/get-started/connectors) for the constructor walkthrough.

These configuration fields are useful early:
...
`@ton/appkit-react` provides hooks for the main AppKit actions and watchers. Mount `AppKitProvider` once at the root inside a TanStack `QueryClientProvider`, then call hooks anywhere below it. See [Installation → Set up React providers](/ecosystem/appkit/get-started/installation/react-app#set-up-react-providers) for the mount.
