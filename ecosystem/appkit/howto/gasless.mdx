...
const feeJettonMaster = config.supportedGasJettons[0].jettonMaster;
...
// Show estimate.fee in the chosen jetton before opening the wallet.
...
{/* TODO(docs/sdk): `sendGaslessTransaction` currently re-estimates internally, so the fee shown from `estimateGasless` may not exactly match what the relayer charges at send. Once the `{ estimate }` overload lands (see GASLESS-TECH-LEAD-BRIEF.md → A8), prefer `sendGaslessTransaction(appKit, { estimate })` and drop the second `feeJettonMaster`/`messages` pair. */}
