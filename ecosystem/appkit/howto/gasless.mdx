...
{/* TODO(docs/sdk): `sendGaslessTransaction` currently re-estimates internally, so the fee shown from `estimateGasless` may not exactly match what the relayer charges at send. Once the `{ estimate }` overload lands (see GASLESS-BRIEF.md → A8), prefer `sendGaslessTransaction(appKit, { estimate })` and drop the second `feeJettonMaster`/`messages` pair. */}
