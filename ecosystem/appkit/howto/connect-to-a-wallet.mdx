...
When the connector configuration depends on values that aren't available at construction time — a user-scoped manifest URL, a feature flag, a dynamically chosen wallet set — register it with the `addConnector` action. It accepts a `Connector` instance or a factory and returns an unregister function.
