...
You need an `AppKit` instance with at least one connector and the React providers mounted. See [Connectors](/ecosystem/appkit/get-started/connectors) for the instance and [Installation → Set up React providers](/ecosystem/appkit/get-started/installation/react-app#set-up-react-providers) for the provider setup.
...
When you need a tailored UI, render the connector list yourself. Iterate `useConnectors`, then call `connect.mutate({ connectorId: connector.id })`. The display name lives at `connector.metadata.name`.
