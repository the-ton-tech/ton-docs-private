  ...
  export function startNftOwnershipMonitoring(
    ...
    const poll = async () => {
      while (isRunning) {
        ...
        const nfts = await getNfts(appKit, { limit: 100 });
...
`createTransferNftTransaction` is async (it returns a `Promise<TransactionRequest>`) and throws when no wallet is selected. A common failure mode is **stale ownership**: the NFT changes owners between the read and the transfer attempt. Read `ownerAddress` again before opening the wallet so the user signs a transfer based on current ownership. If `isOnSale` is true, also check `realOwnerAddress`.
