...
<Warning>
  **Verify NFT authenticity.** Scammers create fake NFTs that mimic popular collections. An NFT item contract can claim any collection address, so reading the `collection` field from the item alone is not sufficient.
...
NFT ownership is tracked through individual NFT item contracts. Unlike jettons, which have a balance, a wallet either owns a specific NFT item or it does not. Each item contract carries an `ownerAddress` that reflects current ownership.
